...
{
    public static class WallJoinWorkflow
    {
        ...
        private sealed class BeautifyReadSnapshot : IDisposable
        {
            ...
            public void Dispose()
            {
                foreach (BeautifySourceSnapshot source in Sources) source.Dispose();
                Sources.Clear();
            }
        }
            ...
            IReadOnlyList<ObjectId> selectedIds)
        {
            ...
            if (validIds.Count == 0 || (settings.StrictMode && validIds.Count != selectedIds.Count))
            {
            ...
            try
            {
                ...
                using (BeautifyReadSnapshot snapshot = ReadBeautifySnapshot(database, validIds, result))
                {
                    ...
                    try
                    {
                        bool snappedToExisting = GeometryKernelAdapter.TryGetConsensusSnapVector(
                            snapshot.Sources.Select(source => source.Entity),
                            nearbyCurves,
                            settings.SnapRadius,
                        ...
                        {
                            targetAnchor = new Point2d(
                                originalAnchor.X + snapVector.X,
                                originalAnchor.Y + snapVector.Y);
                            result.AddWarning(
                                $"BW snap theo consensus của {supportCount} vertex thật thay vì góc bounding box.");
                        }
                        else
                        {
                            targetAnchor = new Point2d(
                    ...
                    if (replacements.Count == 0 ||
                        (settings.StrictMode && replacements.Count != validIds.Count))
                    {
                        if (settings.StrictMode && replacements.Count != validIds.Count)
                        {
                            result.AddWarning("Strict Mode: rollback vì không xử lý được toàn bộ selection.");
                        }
                        result.SkippedCount = selectedIds.Count;
                        result.WriteSummary(editor);
                        return;
            ...
            bool isFinishing)
        {
            ...
            using (DocumentLock documentLock = document.LockDocument())
            {
                try
                {
                    ...
                    if (extracted.Curves.Count == 0 || (settings.StrictMode && extractionIncomplete))
                    {
                        if (settings.StrictMode && extractionIncomplete)
                        {
                            result.AddWarning("Strict Mode: rollback vì có đầu vào không trích xuất được.");
                        }
                        result.SkippedCount = selectedIds.Count;
                        result.WriteSummary(editor);
                        return;
                    ...
                    if (pendingEntities.Count == 0 ||
                        (settings.StrictMode && (failedClusters > 0 || failedOffsets > 0)))
                    {
                        if (settings.StrictMode && (failedClusters > 0 || failedOffsets > 0))
                        {
                            result.AddWarning(
            ...
            out int failedOffsets)
        {
            ...
            {
                BoundaryBuildResult boundaryResult = GeometryPipeline.ProcessClusterToBoundaries(
                    cluster,
                    settings.GapTolerance,
                    settings.VertexTolerance,
                    logger);
            ...
            WallOperationResult result)
        {
            ...
            {
                if (IsUsableObjectId(database, id)) valid.Add(id);
                else result.AddWarning("Bỏ qua ObjectId null, erased hoặc thuộc database khác.");
            }
            return valid;
        }
        ...
        {
            return id != ObjectId.Null && id.IsValid && !id.IsErased && id.Database == database;
        }
        ...
        private static void DisposeReplacements(IEnumerable<Replacement> replacements)
        {
            foreach (Replacement replacement in replacements)
            {
                ...
                if (entity != null && !entity.IsDisposed && entity.Database == null) entity.Dispose();
            }
        }
